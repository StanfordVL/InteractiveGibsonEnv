...
- Your output format must strictly follow the json format like this:{"operator": <operator name>, "params": <params>} where operator name is either state primitives or connectives, and params can be either state primitives or connectives. For example, {"operator": "inside", "params": ["coin_1", "jar_1"], {"operator": "not", "params": [{"operator": "open", "params": ["jar_1"]}]}
'''
...
## Output: Based on initial states, achieve final goal states logically and reasonably. It does not matter which state should be satisfied first, as long as all goal states can be satisfied at the end. Output concrete states and do not use quantifiers like "forall" or "exists". Make sure your output follows the json format, which is a list of dictionaries, strictly without irrelevant information.
{"operator": "open", "params": ["fridge.97"]}
{"operator": "and", "params": [{"operator": "ontop", "params": ["strawberry.0", "countertop.84"]}, {"operator": "ontop", "params": ["peach.0", "countertop.84"]}]}
{"operator": "holds_rh", "params": ["carving_knife.0"]}
{"operator": "and", "params": [{"operator": "sliced", "params": ["strawberry.0"]}, {"operator": "sliced", "params": ["peach.0"]}]}
{"operator": "and", "params": [{"operator": "inside", "params": ["strawberry.0", "jar.0"]}, {"operator": "not", "params": [{"operator": "inside", "params": ["peach.0", "jar.0"]}]}]}
{"operator": "and", "params": [{"operator": "inside", "params": ["peach.0", "jar.1"]}, {"operator": "not", "params": [{"operator": "inside", "params": ["strawberry.0", "jar.1"]}]}]}
{"operator": "and", "params": [{"operator": "not", "params": [{"operator": "open", "params": ["jar.0"]}]}, {"operator": "not", "params": [{"operator": "open", "params": ["jar.1"]}]}
'''
...
if __name__ == '__main__':
    prompt_file_path = 'F:\\Projects\\Research\\embodiedAI\\kangrui\\iGibson\\igibson\\evaluation\\eval_subgoal_plan\\resources\\subgoal_plan_prompt_s_ltl_json.json'
